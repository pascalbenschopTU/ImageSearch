...
class ViTExtractor:
    ...
    def __init__(
        ...
        if getattr(self.model, "_is_dinov3", False):
            # DINOv3 ViTs are 16x16 patch models; keep natural stride==patch_size
            # and avoid patch_vit_resolution (not compatible with HF module internals).
            self.p = self.model.config.patch_size           # 16
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        :param model_type: a string specifying which model to load. [dino_vits8 | dino_vits16 | dino_vitb8 |
                           ...
                           # +  dinov3_vits16 | dinov3_vitb16 | dinov3_vitl16 | dinov3_vith14 | dinov3_vit7b16
        :return: the model
        """
        if "dinov3" in model_type:
            assert _DINOV3_OK, "Install transformers>=4.53 and timm."
            # Map simple aliases → HF IDs if you like; otherwise require full HF id
            hf_id = {
            ...
            model._hf_processor = AutoImageProcessor.from_pretrained(hf_id)
    ...
    def preprocess(self, image_path, load_size=None):
        ...
        if getattr(self.model, "_is_dinov3", False):
            # HF processor handles resize & normalization correctly for DINOv3.
            size = load_size or 224
            # normalize size to an int for HF; if a tuple was passed, pick the larger side
            if isinstance(size, (tuple, list)):
                # You can choose max or min; using max keeps more detail when the image is not square.
                size = int(max(size))
                
            inputs = self.model._hf_processor(images=pil_image, return_tensors="pt", size=size)
            prep_img = inputs["pixel_values"]  # [1,3,H,W], default 224x224
            # mirror existing API return: pil resized to the same size
            pil_resized = pil_image.resize((size, size), Image.BICUBIC)
            return prep_img, pil_resized
        else:
    ...
    def extract_descriptors(
        ...
        """
        if getattr(self.model, "_is_dinov3", False):
            assert facet in ["token", "key", "query", "value"]  # but we’ll use token path
            assert not bin, "bin not supported for DINOv3 in this extractor."
            # Forward to get tokens
            outputs = self.model(batch)  # HF ModelOutput
            x = outputs.last_hidden_state  # [B, 1+R+T, D]
            num_reg = getattr(self.model.config, "num_register_tokens", 0)
            # Strip CLS + register tokens
            x = x[:, 1 + num_reg:, :]     # [B, T, D] patch tokens only
            # Save grid info for search_engine mapping
            H, W = batch.shape[-2:]
            h_tokens, w_tokens = H // self.p, W // self.p
            self.load_size = (H, W)
            self.num_patches = (h_tokens, w_tokens)
            # Match original return shape: [B, 1, t, D]
            desc = x.unsqueeze(1)
            return desc
